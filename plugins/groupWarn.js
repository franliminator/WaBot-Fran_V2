...
let handler = async (m, { conn, text }) => {
  ...
  if(!text && !m.quoted) return conn.reply(m.chat, `*[ ERROR ADVERTENCIA ]*\n\nEtiquete al usuario, anote el número o responda al miembro que desea dar _advertencia_`, m)
  //let exists = await conn.isOnWhatsApp(number)
  // if (exists) return conn.reply(m.chat, `*Nomor target tidak terdaftar di WhatsApp*`, m)
  if(isNaN(number)) return conn.reply(m.chat, `*[ ERROR ADVERTENCIA ]*\n\nEl número que ingresaste no es válido !`, m)
  if(number.length > 15) return conn.reply(m.chat, `*[ ERROR ADVERTENCIA ]*\n\nEl número que ingresaste no es válido !`, m)
  try {
		if(text) {
...
handler.help = ['advertencia', 'advertencia *(contestar msg)*','advertencia *@usuario*']
handler.tags = ['group admin']
handler.command = /^advertencia$/i
handler.admin = true
handler.group = true
